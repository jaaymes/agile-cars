...
export default function MarcasPage() {
  ...
  const handleDeleteRow = async (id: number) => {
    ...
    const newFranqueados = modelosVersao.filter(
      (colaborador) => colaborador.idModeloVersao !== id
    );
    setModelosVersao(newFranqueados);
  };

  const handleEditRow = (id: number) => {
    push(`/admin/modelo-versao?id=${id}`);
  };
  ...
  const handleGetAllMarcas = async () => {
    ...
    setMarcas(
      marcas?.map((marca: { descricaoMarca: any; idMarca: any }) => ({
  ...
  const handleGetAllModelos = async () => {
    ...
    setModelos(
      modelos?.map((modelo: { descricaoModelo: any; idModelo: any }) => ({
        label: modelo.descricaoModelo,
        id: modelo.idModelo,
      }))
    );
    setIsLoading(false);
  ...
  const handleGetModelosVersao = async () => {
    ...
    if (modelos) {
      setDataInPage(
        modelos?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
      );
            ...
            >
              <FormControl fullWidth>
                <Autocomplete
                  ...
                  clearIcon={null}
                  value={
                    marcas?.find((marca) => marca.id === selectIdMarca) || null
                  }
              ...
              <FormControl fullWidth>
                <Autocomplete
                  ...
                  disablePortal
                  value={
                    modelos?.find((modelos) => modelos.id === selectIdModelo) ||
                    null
                  }
                  onChange={(event, value) =>
                    ...
                    <TableBody>
                      {dataInPage.map((row) => (
                        <ModelosVersaoCustomTable
                          ...
                          row={row}
                          onDeleteRow={() =>
                            handleDeleteRow(row.idModeloVersao)
                          }
                      ...
                      <TableEmptyRows
                        emptyRows={emptyRows(
                          page,
                          rowsPerPage,
                          modelosVersao?.length
                        )}
                      />

                      <TableNoData
                        isNotFound={!dataInPage?.length}
                        text="Escolha Marca e Modelo"
                      />
                    </TableBody>
                  }
                </Table>
