...
export default function ShopProductCard({ product }: Props) {
  ...
  const handleRedirectWhatsapp = () => {
    window.open(
      `https://api.whatsapp.com/send?phone=${product.whatsapp.replace(
        "+",
        ""
      )}&text=Olá, gostaria de mais informações sobre o veículo
    %0A%0A
  ...
  useEffect(() => {
    if (
      window.location.hostname.toLocaleLowerCase().indexOf("agileveiculos") <=
      -1
    )
      setRodandoLocal(true);
    else setRodandoLocal(false);
  }, [router, product]);
  ...
  return (
    ...
    >
      <Box sx={{ position: "relative", p: 1 }}>
        <Image
          alt={product.chassi}
          src={product?.foto1 ? product?.foto1 : "/sem-imagem.png"}
          ratio="1/1"
          sx={{ borderRadius: 1.5 }}
        />
      </Box>
      <Stack
        spacing={1}
        sx={{ px: 2, display: "flex", height: "100%" }}
      ...
      >
        <Stack direction="column" spacing={0.5}>
          ...
          <Box
            component="span"
            sx={{ fontSize: "12px" }}
          >{`Cartegoria: ${product.descricaoCategoria}`}</Box>
          <Box
            component="span"
            sx={{ fontSize: "12px" }}
          >{`${product.fab}/${product.mod}`}</Box>
          <Box
            component="span"
            sx={{ fontSize: "12px" }}
          >{`KM: ${product.km}`}</Box>
        ...
        >
          ...
          <Link
            href={`/shop/veiculo?id=${product?.idVeiculo}&idFranqueado=${product?.idFranqueado}`}
            passHref
          >
            {/* @ts-ignore */}
